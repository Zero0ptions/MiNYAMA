...
Already written to `C:\Users\user\.config\opencode\opencode.jsonc`. Re-create only if missing:
...
- Prerequisite: `uvx` must be on PATH (verified present at `C:\Users\user\AppData\Local\hermes\bin\uvx.exe`).
